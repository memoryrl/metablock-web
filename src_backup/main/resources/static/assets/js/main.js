(function ($) {
    ...
    /*====== Sidebar Download [custom] ======*/
    function miniCartCustom() {
        var navbarTrigger = $('.download-cart-active'),
            endTrigger = $('.download-cart-close'),
            container = $('.sidebar-download-active'),
            wrapper = $('.main-wrapper');
    ...
    function downloadCartButtonClickEvent() {
        if (sessionStorage.getItem("_CHECK_MYDOWNLOAD_") == 0) {
            const headerDiv = document.getElementById("__HEADER__")
            const bellAlarm = headerDiv.querySelector("#_BELL_ALARM_")
            bellAlarm.innerHTML = ""
            sessionStorage.removeItem("_CSV_DOWNLOADING_");
            sessionStorage.setItem("_CHECK_MYDOWNLOAD_", "1");
        }
